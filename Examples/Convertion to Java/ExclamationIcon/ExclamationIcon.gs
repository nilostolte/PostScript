...
% Author: Nilo Stolte
%
% License: this file is provided as is and can be copied, modified, or distributed with some restrictions. Code snippets extracted from this file are authorized and totally are free EXCEPT for the code identified by the mentions "CODE RESERVED - CANNOT BE COPIED" and "END OF RESERVED CODE". This code cannot be used outside the context of this file and is copyrighted by Nilo Stolte. When the entired file is used as is, not copied as a snippet, all modifications must be commented with the proper credit under the "Modified by" section below EXCEPT when the modoficition is just a different vector art (in this case the current vector design of the warning sign with exclamation point can be substituted by another design with the prurpose to convert that design into Java). Following the credit, a brief description of the modification must be stated to differentiate from original code. Package information, comments, changing variables names and other changes that do not modify or do not add any functionality are not considered modifications and don't need to be commented. Authorized snippets can be used without this comment, no credits are needed. 
%
% Modified by:
...
/psdefinitions {
	...
	/rad {
		...
		/FunctionType 2
		/Domain [ 0 1 ]
		/C0 16 -1 roll dup type /integertype eq { c255 } if
		/C1 17 -1 roll dup type /integertype eq { c255 } if
		/N 1
		>>
		/Extend [ false false ]
		/AntiAlias true
...
/javadefinitions {
	...
	% function to create an array of paths
	/p {
		(   static private Path2D.Float ) ws exch ws ([] = {\n) ws
		{
			(      ) ws ws (\(\),\n) ws
	...
	/s {
		line begin ° (new GradientPaint\() + 
			dup aload pop pop pop exch # (f,) + # (f,) +
			exch (new Color\() + aload pop 3 -1 roll # (,) + exch # (,) + # (\),) +
			aload pop 4 -2 roll pop pop exch # (f,) + # (f,) +
			(new Color\() + aload pop 3 -1 roll # (,) + exch # (,) + # (\)\),\n) + =
		end 
	} def
	/rad {
	...
	% function to create an array of gradients
	/g {
	...
	/drawstrokeshade {
		exch
		2 index ws (g.setPaint\() ws ws ([i]\);\n) ws
		exch ws (g.draw\() ws ws (\);\n) ws
	} def
	...
	/drawpath {
		2 index ws (g.setPaint(pOrg);\n) ws 			% set the paint with original paint
